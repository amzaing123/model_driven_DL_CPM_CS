...
def pwlin_grid(r_, rvar_, theta_):
    """
    ...
    theta1, theta2, theta3, theta4, theta5 = theta_[0], theta_[1], theta_[2], theta_[3], theta_[4]

    abs_r_ = tf.abs(r_)
    sgn_r_ = tf.sign(r_)

    sigma_ = tf.sqrt(rvar_)
    sigma_ = tf.expand_dims(sigma_, axis=2)

    thresh1 = theta1 * sigma_
    thresh2 = theta2 * sigma_

    # --- 2. 创建三个区域的布尔掩码 (Masks) ---
    mask1 = abs_r_ <= thresh1
    mask2 = (abs_r_ > thresh1) & (abs_r_ <= thresh2)
    mask3 = abs_r_ > thresh2

    mask1_f = tf.cast(mask1, tf.float64)
    mask2_f = tf.cast(mask2, tf.float64)
    mask3_f = tf.cast(mask3, tf.float64)
    mask1_c = tf.cast(mask1, tf.complex128)
    mask2_c = tf.cast(mask2, tf.complex128)
    mask3_c = tf.cast(mask3, tf.complex128)

    # --- 3. 根据公式计算每个区域的 xhat ---

    # Case 1: |r| <= θ1*σ  =>  xhat = θ3 * r
    # *** 核心修正：在乘法前将 theta3 转换为 complex128 ***
    xhat1 = tf.cast(theta3, tf.complex128) * r_

    # Case 2: θ1*σ < |r| <= θ2*σ
    val2 = theta4 * (abs_r_ - thresh1) + theta3 * thresh1
    xhat2 = sgn_r_ * tf.cast(val2, tf.complex128)

    # Case 3: |r| > θ2*σ
    val3 = theta5 * (abs_r_ - thresh2) + theta4 * (thresh2 - thresh1) + theta3 * thresh1
    xhat3 = sgn_r_ * tf.cast(val3, tf.complex128)

    xhat_ = (xhat1 * mask1_c) + (xhat2 * mask2_c) + (xhat3 * mask3_c)

    # --- 4. 解析计算导数 dxdr ---
    dxdr_val = (mask1_f * theta3) + (mask2_f * theta4) + (mask3_f * theta5)
    dxdr_avg_f = tf.reduce_mean(dxdr_val, axis=[1, 2])
    dxdr_ = tf.cast(dxdr_avg_f, tf.complex128)
