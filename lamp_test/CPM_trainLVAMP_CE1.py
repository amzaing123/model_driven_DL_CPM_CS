...
for snr in SNRrange:
    ...
    savenetworkfilename = type + '_' + shrink + '_' + M_N_K + '_train_' +'layer_number'+ str(T)+'_' + str(snr)+'dB.mat'
    ...
    layers, h_, A = CPM_LVAMP_CE_Network1.build_LVAMP_dense(M=M, N=N,  K=K,  snr=snr, T=T, shrink=shrink)
    training_stages = CPM_LVAMP_CE_Network1.setup_training(layers, h_, A,  K, N, M, trinit=1e-3, refinements=(0.5,))
    sess = CPM_LVAMP_CE_Network1.do_training(h_=h_, training_stages=training_stages, A=A, savefile=savenetworkfilename,
