...
def build_LVAMP_dense(M, N, K,  snr, T, shrink):
    ...
    xhat_,dxdr_ = eta(rhat_,rvar_,theta_ )  #xhat_nl_,complex128  alpha_nl_,complex128 # xhat_ (None, M, K)
    ...
    for t in range(2,T+1):
        ...
        #     rvar_ = rvar_ * gain_v
        with tf.device('/cpu:0'):
            gain_v1 = tf.divide(1, 1 - dxdr_)  #旧版本 论文可能数学模型可能存在问题
            # gain_v1 = tf.divide(1,  dxdr_)
        gain_v2 = gain_v1 * dxdr_#旧版本 论文可能数学模型可能存在问题
        # gain_v2 = gain_v1 * (1-dxdr_)
        gain_v = tf.sqrt(gain_v2)
        rvar_ = rvar_ * gain_v
        ...
        xhat_,dxdr_ = eta(rhat_,rvar_,theta_ )#rvar_ float64   rhat_ complex128 (None,M,K)
        # print("xhat_ dtype:", xhat_.dtype, "xhat_ shape:", xhat_.shape)
        # print("dxdr_ dtype:", dxdr_.dtype, "dxdr_ shape:", dxdr_.shape)
