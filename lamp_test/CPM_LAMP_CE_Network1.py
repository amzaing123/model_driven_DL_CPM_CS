...
def build_LAMP(M, N, K,  snr, T, shrink, untied):#自定义LAMP网络，训练网络的每一层和后续网络的每一层
    ...
    # xhat_, dxdr_1 = eta(Bv_1, rvar_1, K, theta_)  # xhat_ (None, M, K) xhat_1   此为bg是收缩函数
    ...
    # print("b_1 dtype:", b_1.dtype, "shape:", b_1.shape)
    vb = tf.reshape(v0, [tf.shape(h_)[0], M * K])  # (None, M*K)
    # print("vb dtype:", vb.dtype, "shape:", vb.shape)
    bv = tf.multiply(b_1, vb)  # (None,M*K) b1*v0
    # print("bv dtype:", bv.dtype, "shape:", bv.shape)
    ...
    for t in range(2, T+1): ###从2到T，2，3，4，5
        ...
        # xhat_, dxdr_ = eta(rhat_, rvar_, K, theta_)  # xhat_ (None,M,K)
        xhat_, dxdr_ = eta(rhat_, rvar_, theta_)  # xhat_ (None,M,K)
