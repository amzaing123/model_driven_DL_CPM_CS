...
for snr in SNRrange:
    ...
    layers, h_, A = CPM_LAMP_CE_Network.build_LAMP(M=M, N=N,  K=K,  snr=snr, T=T, shrink=shrink, untied=False)
    training_stages = CPM_LAMP_CE_Network.setup_training(layers, h_, A,  K, N, M, trinit=1e-3, refinements=(0.5,))
    sess = CPM_LAMP_CE_Network.do_training(h_=h_, training_stages=training_stages, A=A, savefile=savenetworkfilename,
